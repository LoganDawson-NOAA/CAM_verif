...
def write_regrid_job(job_script):
    ...
    with open(job_script,'w') as f:
        f.write("#!/bin/bash\n")
        f.write("#PBS -N prep_mrms_"+valid_time+"\n")
        f.write("#PBS -o "+RUN_DIR+"/regrid_mrms.out\n")
        f.write("#PBS -e "+RUN_DIR+"/regrid_mrms.err\n")
        f.write("#PBS -l walltime=00:30:00\n")
        f.write("#PBS -l select=1:ncpus=32:mem=5GB\n")
      ...
      # f.write("#PBS -e "+RUN_DIR+"/regrid_mrms.err\n")
      # f.write("#PBS -R \"affinity[core]\"\n")
      # f.write("#PBS -R span[ptile=16]\n")
      # f.write("#PBS -n 80\n")
      # f.write("#PBS -R \"rusage[mem=1500]\"\n")
        ...
        f.write("\n")
        f.write("   run_metplus.py -c "+METPLUS_CONFIG_DIR+"/ldawson_"+system+".conf "+USE_CASE_DIR+"/RegridDataPlane_obsMRMS_${MRMS_PRODUCT}_toG227.conf\n")

        f.write("\n")
        f.write("   echo \"creating 40-km max field for MRMS ${MRMS_PRODUCT}\"\n")
        f.write("\n")
        f.write("   run_metplus.py -c "+METPLUS_CONFIG_DIR+"/ldawson_"+system+".conf "+USE_CASE_DIR+"/RegridDataPlane_obsMRMS_${MRMS_PRODUCT}_NBMAX.conf\n")
        f.write("\n")
        f.write("done")
...
#       f.write("python /gpfs/dell2/emc/verification/save/Logan.Dawson/CAM_verif/mrms_obs_fields.py\n")
        ...
        f.write("\n")
        f.write("   run_metplus.py -c "+METPLUS_CONFIG_DIR+"/ldawson_"+system+".conf "+USE_CASE_DIR+"/RegridDataPlane_obsMRMS_${MRMS_PRODUCT}_PPF.conf\n")
        f.write("\n")
        f.write("done")
        f.write("\n")
...
MRMS_PROD_DIR = '/lfs/h1/ops/prod/dcom/ldmdata/obs/upperair/mrms/conus'
RUN_DIR = '/lfs/h2/emc/ptmp/'+os.environ['USER']+'/CAM_verif/METplus/runscripts/MRMS/'+valid_time
