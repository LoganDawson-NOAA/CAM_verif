...
def write_metplus_job():
    ...
    with open(RUN_DIR+'/verif_'+model+'_'+field+'.sh','w') as f:
        ...
        f.write(". ~/dots/dot.for.metplus-3.0\n")
        f.write("\n")
        f.write("cd "+RUN_DIR+"\n")
        f.write("\n")
...
HiResModels = ['CONUSARW','CONUSARW2','CONUSFV3','CONUSHREF_MEAN','CONUSHREFX_MEAN','CONUSHREF_PMMN','CONUSHREFX_PMMN','CONUSHREF_PROB','CONUSHREFX_PROB','CONUSNEST','CONUSNMMB','FV3SAR','FV3SARDA','FV3SARX','HRRR','HRRRX']
...
for use_case in use_cases:
    ...
    if use_case == 'grid2grid_mrms':
        ...
        models = ['CONUSARW','CONUSARW2','CONUSFV3','CONUSHREF_PMMN','CONUSHREFX_PMMN','CONUSHREF_PROB','CONUSHREFX_PROB','CONUSNEST','CONUSNMMB','FV3SAR','FV3SARDA','FV3SARX','HRRR']
    elif use_case == 'surrogate_severe':
        vx_grid = 'G211'
        stat_analysis_conf = 'StatAnalysis_fcstSSPF_obsPPF_gatherByHour.conf'
        models = ['CONUSARW','CONUSARW2','CONUSFV3','CONUSHREF_PROB','CONUSHREFX_PROB','CONUSNEST','CONUSNMMB','FV3SAR','FV3SARDA','FV3SARX','HRRR']
    elif use_case == 'grid2obs':
        vx_grid = 'G104'
    ...
    MASK_DIR = os.path.join(NOSCRUB_DIR,'masks',vx_grid)

    # Define directory for StatAnalysis output
    STAT_ANALYSIS_DIR = os.path.join(SCRIPT_DIR,'metplus.out','stat_analysis',use_case,valid_time.strftime('%Y%m%d'))
    ...
    for model in models:
        ...
        RUN_DIR = os.path.join(SCRIPT_DIR,'runscripts',use_case,model,valid_time.strftime('%Y%m%d%H'))
        ...
        if str.upper(model[0:2]) == 'AK' or str.upper(model[-2:]) == 'AK':
            ...
            spc_outlooks = ['day1otlk_'+day1_issued+'_1200','day2otlk_'+day2_issued+'_1730','day3otlk_'+day3_issued+'_0730']
        ...
        # Write METplus jobs for radar verification
        if use_case == 'grid2grid_mrms':
        ...
        # Write METplus jobs for surrogate severe verification
        elif use_case == 'surrogate_severe':
        ...
        # Write METplus jobs for grid2obs verification
        elif use_case == 'grid2obs':
